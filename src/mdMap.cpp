...
bool mdMap::update(float dt) {
	if (map_loaded)
	{
		...
		{
			if (selected_map == 1)
				App->audio->playMusic(App->audio->loadMusic("SFX/stage music/BGM_3.ogg"));
			else if (selected_map == 2)
				App->audio->playMusic(App->audio->loadMusic("SFX/stage music/BGM_1.ogg"));
			else if (selected_map == 3)
				App->audio->playMusic(App->audio->loadMusic("SFX/stage music/BGM_2.ogg"));
			change_music = false;
		}
