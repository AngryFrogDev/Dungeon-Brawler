...
void mdMap::draw() {
		if (selected_map == 0) {
			//Blit background
			App->render->drawSprite(1, data.background_image, 0, 0, (const SDL_Rect*)0, 6, false, 0.3);
			App->render->drawSprite(1, data.background_image, 0, 0, (const SDL_Rect*)0, 6, false, 0.3);
			//Blit map
			App->render->drawSprite(2, data.map_image, 0, 0, (const SDL_Rect*)0, 5, false);
			App->render->drawSprite(2, data.map_image, 0, 0, (const SDL_Rect*)0, 5, false);
		}
...
bool mdMap::loadMap(int mapIndex) {
	...
	//else {
		...
		if (mapIndex == 0) {		// Load map characteristics, (Provisional, should be done thorugh xml)
			data.map_image = App->textures->load("assets/village.png");
			data.background_image = App->textures->load("assets/village_background.png");
			parallax_speed = 1;
			selected_map = 0;
