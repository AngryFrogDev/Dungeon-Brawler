	...
	for (std::list<collider*>::iterator it = colliders.begin(); it != colliders.end(); ++it) {
		...
		switch (c->type)
		{
			case COLLIDER_NONE: // white
				App->render->drawQuad( 1, c->rect, 255, 255, 255, alpha,true, false);
				break;
			case HITBOX:
				App->render->drawQuad( 1, c->rect, 255, 0, 0, alpha, true, false);
				break;
			case HURTBOX:
				App->render->drawQuad( 1, c->rect, 0, 0, 255, alpha, true, false);
				break;
			case PUSHBOX:
				App->render->drawQuad( 1, c->rect, 255, 127, 80, alpha, true, false);
				break;
			case PROJECTILE_HITBOX:
				App->render->drawQuad( 1, c->rect, 255, 255, 255, alpha, true, false);
				break;
			case PROJECTILE_INVENCIBLE_HURTBOX:
				App->render->drawQuad( 1, c->rect, 0, 255, 0, alpha, true, false);
				break;
		}
	}
}
