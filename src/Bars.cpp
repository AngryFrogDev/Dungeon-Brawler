...
void Bars::draw()	{
	...
	updateBarGauge();
	App->render->drawSprite(4, App->gui->getAtlas(), position.first, position.second, &bar_rect, 2, flipped);
	App->render->drawSprite(5, App->gui->getAtlas(), position.first + relative_pos.x + aux_bar_pos, position.second + relative_pos.y, &current_gauge_rect, 2, flipped);
}
